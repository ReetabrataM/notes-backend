...
const PROMPTS = {
    ...
    interview_questions: (text) => `Generate 6 realistic interview questions based on this material, each followed by a brief model answer. Format as "Q: ...\\nA: ...":\n\n${text}`,
};
...
function extractGoogleErrorMessage(error) {
    ...
    if (googleMessage)
        return `Gemini API error (HTTP ${status}): ${googleMessage}`;
...
class AIService {
    assertConfigured() {
        if (!env_1.env.aiConfigured) {
            throw apiResponse_1.ApiError.badRequest('AI features require a GEMINI_API_KEY in the backend .env file. Get a free key at https://aistudio.google.com/app/apikey, add it, and restart the server.');
        }
    }
    /** Returns null (never throws) if AI isn't configured — embeddings are a background enhancement, not a blocking requirement */
    async embed(text) {
        if (!env_1.env.aiConfigured || !text?.trim())
            return null;
        try {
            const response = await axios_1.default.post(`${GEMINI_BASE}/${env_1.env.ai.embeddingModel}:embedContent?key=${env_1.env.ai.apiKey}`, { content: { parts: [{ text: text.slice(0, 8000) }] } }, { headers: { 'Content-Type': 'application/json' }, timeout: 15000 });
            ...
            logger_1.logger.error('Gemini embedding request failed', { error: extractGoogleErrorMessage(error) });
            return null;
        }
    }
    /**
    ...
    async chatCompletion(messages) {
        ...
        try {
            const response = await axios_1.default.post(`${GEMINI_BASE}/${env_1.env.ai.model}:generateContent?key=${env_1.env.ai.apiKey}`, {
            ...
            if (!candidate) {
                ...
                throw apiResponse_1.ApiError.badRequest(blockReason
            ...
            logger_1.logger.error('Gemini chat completion failed', {
                message: realMessage,
            ...
            throw new apiResponse_1.ApiError(502, realMessage);
        }
    }
    async run(action, text, opts) {
        this.assertConfigured();
        if (!text?.trim())
            throw apiResponse_1.ApiError.badRequest('There is no note content to process');
     ...
     */
    async generateNoteTemplate(description) {
        this.assertConfigured();
        if (!description?.trim())
            throw apiResponse_1.ApiError.badRequest('Describe what kind of template you want');
